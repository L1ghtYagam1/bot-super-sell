...
- Поиск по нескольким страницам API для более точного отбора по `tag_length`
...
Бот просматривает до 10 страниц API и возвращает первые подходящие результаты по твоим фильтрам. При повторном /poisk продолжает с следующей страницы и не показывает уже выданные теги повторно. Сбросить прогресс и историю можно через /sbros.
